...
function showproctype(t){	
	...
	p1.css('top', y-80);
	$('#proctypeBody').html('<center><img src="img/ajax-loaders/ajax-loader-1.gif" border=0> Получение данных <img src="img/ajax-loaders/ajax-loader-1.gif" border=0></center>');
	p1.show();
	$(document).click( function(event){
		if( $(event.target).closest(".popup_clean1").length || $(event.target).closest($(cur_proctype_field)).length ) 
			return;
		$(p1).css('display', 'none');
		event.stopPropagation();		
	});
	...
	X.onreadystatechange = function() {
		if (X.readyState == 4) {				
			if(X.status == 200) {
				$('#proctypeBody').html(X.responseText);	
				fillproctype(t.value);			
			} else $('#proctypeBody').html('<div class="alert alert-error">Ошибка '+X.status+'</div>');
		}
	};
	X.send();
}