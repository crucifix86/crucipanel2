...
if ($goreg==1 && $register_active) {
	...
	if ($error==0){
		...
		if (substr($result, 0, 2)=='ok') {
			...
			$text.=getdata('Пароль',$password);
			$text.=getdata('Вопрос',$postquestion);
			$text.=getdata('Ответ',$postanswer);
			$text.=getdata('Имя',$realname);
			$text.='</table></div><br>';
			if ($email_confirm) {
				$confirm_link = GetLink().'register.php?activate='.mycrypt(encode(sprintf('%d|%s|%d', $servid, $login, $realid)));
				$text.=sprintf('<p>Для активации аккаунта перейдтите по ссылке: <a href="%s" targer="_blank">%s</a></p></br>', $confirm_link, $confirm_link);
			}
			$text.='
