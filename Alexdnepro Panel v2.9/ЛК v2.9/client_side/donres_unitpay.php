...
switch($method) {
	...
	$item->id = $bonus_id; $item->count = $bonus_count; $item->max_count = $bonus_max_count; $item->data = pack('H*', $bonus_data); $item->proctype = $bonus_proctype; $item->mask = $bonus_mask;
	if ($bonus_expire > 0) $item->expire = time()+$bonus_expire; else $item->expire = 0;
	// Bonux Fix begin	
	...
	query2mysql(sprintf("UPDATE `donate_unitpay` SET `act_bonus`=%d, `status`=%d, `out_summ`='%s', `profit`='%s', `money`='%s', `bonus_money`='%s', `operator`='%s', `paymentType`='%s', `phone`='%s', `unitpayId`='%s' WHERE `id`=%d", $don['act_bonus'], $status, $params['sum'], $params['profit'], $don['moneycount'], $don['bonus'], $db->real_escape_string($params['operator']), $db->real_escape_string($params['paymentType']), $db->real_escape_string($params['phone']), $db->real_escape_string($params['unitpayId']), $don['inv_id']));
	$res = CurlPage($don, 20);
	if ($res == 'NeedNewID') {
