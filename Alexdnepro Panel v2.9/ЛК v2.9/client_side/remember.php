...
function SendRequest($poststr, $second = false){
	...
	if (!$second) {
		...
		if ($result=='wrongsendemail') $err = $wrongsend; else
		if ($result=='time') $err = '<font color="#900">Ваш айпи забанен на 10 минут, за превышение лимита запросов</font><br>'; else
		if ($result=='ok') {
			if (@send_mail($result1['to'], $result1['subj'], $result1['text']))	$err = ''; else $err = $wrongsend;
		...
		if ($result=='ok') {
			@send_mail($result1['to'], $result1['subj'], $result1['text']);
...
function callServer(){
	...
	$.ajax({
	  ...
	  url: 'remember.php',
	  cache: false,
	  data: 'ch=1&login='+F.login.value+'&r='+Math.random(),		  
...
function callServer1(){
	...
	$.ajax({
	  ...
	  url: 'remember.php',
	  cache: false,
	  data: 'rm=1&email='+F.email.value+'&r='+Math.random(),		  
	  success: function(html){
