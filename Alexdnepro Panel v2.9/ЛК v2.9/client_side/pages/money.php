...
?>
	...
	function checksend(){				
		...
		if (document.sendform.captcha.value.length != 4) { alert ("Введите код безопасности");
		} else {
	...
	if ($allow_lk_transfer) { ?>
		<form name="sendform" method="post" action="?op=act&n=10&num=0&rand=<?=time()?>">
			...
			<table class="table table-bordered table-striped table-condensed" style="width:300px">
				...
				<tr>
					<td><div title="Капча" data-rel="tooltip" style="padding-top:2px"><img src="captcha/?<?php echo session_name()?>=<?php echo session_id()?>&tt=<?=doubleval(microtime())?>" id="image" onClick="iload()" border="0"> <input name="captcha" class="config_itemid" type="text" maxlength="4" placeholder="Капча"></div></td>
				</tr>
				<tr>
