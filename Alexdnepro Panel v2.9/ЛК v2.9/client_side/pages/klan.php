			...
			if ($a['factions']) {
				...
				foreach ($a['factions'] as $i => $val){
					...
					foreach ($val['items'] as $i1 => $val1){
						...
						$costitem = '';
						if ($val1['cost_item_count']) $costitem = '<span class="label label-important">'.$val1['cost_item_count'].'</span><span class="label label-inverse">'.$val1['cost_name'].'</span>';
						printf('<span class="label label-important">%d</span><span class="label label-success">%s</span> за %s %s | ', $val1['count'], $val1['item'], $cost, $costitem);
					}
					echo '<br>';
				}
			}
					...
					if (count($val['klanitems'])>0) {
						foreach($val['klanitems'] as $i1 => $val1) {
							...
							if ($val1['itemid'] == 0 || $val1['cost_item_count'] == 0) $costitem = ''; else
								$costitem = ' за '.$val1['cost_item_count'].' '.$val1['cost_name'];
							$klanit.='<a class="btn btn-mini btn-inverse" style="margin-top:2px" href="index.php?op=act&n=12&num='.$val['roleindex'].'&artid='.$val1['id'].'&rand='.time().'" '.$confirm1.'>Купить '.$val1['count'].' '.$val1['name'].$costitem.$cost.'</a><br>';
						}
					}					
			...
			<div class="box span12">
				...
				<div class="box-content">
					...
					<tr>
						...
						<th><input name="search_mask" value="Mask..." class="search_init" /></th>
