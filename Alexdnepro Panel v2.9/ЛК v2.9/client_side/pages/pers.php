<?php
	...
	if ($a['errorcode'] == 0) {
		...
		$factrole = array(2=>"Мастер",3=>"Маршал",4=>"Майор",5=>"Капитан",6=>"");
		...
		if ($a['roles_count'] > 0){
			...
			foreach ($a['roles'] as $i => $val){
				...
				$active_buffs = '';
				if (count($val['active_buffs']) > 0) {
					foreach($val['active_buffs'] as $i1 => $val1){
						$b_icon = ($val1['icon']=='')?'':'<img src="data:image/png;base64,'.$val1['icon'].'" border="0" class="imcnt"> ';
						$active_buffs .= sprintf('%s%s <font color="00ff00">%s</font><br>', $b_icon, $val1['name'], GetTime($val1['time'], 'ffff00'));
					}
					$active_buffs .= '<hr>';
				}
				if ($val['factionid'] == 0) {
				...
				$pink = GetTime($val['pariah_time'], 'ffff00', false);	
				$forbid_txt = '';			
				...
				$lvlupbut = '';
				if ($lvlup_enable && $val['level'] < $lvlup_maxlvl) {					
					$lvlupbut = '<a class="btn btn-mini btn-inverse wfull" href="#" onclick="ShowSubActWnd('.$val['index'].', \'Покупка уровня\',\'buylvl\')">Купить уровень...</a><br>';
				}
				if ($tp2world_enable && $val['worldtag'] != 1 && $val['worldtag'] != 204) $dungeon_fix = '<a class="btn btn-mini btn-danger wfull" href="index.php?op=act&n=1&num='.$val['index'].'&rand='.time().'" '.$confirm.'">Вытащить из данжа'.ShowCost($tp2worldcost, false, false, false).'</a><br>'; else $dungeon_fix = '';
				if ($fixsp_enable && $val['sp'] < 0) $sp_fix = '<a class="btn btn-mini btn-danger wfull" href="index.php?op=act&n=15&num='.$val['index'].'&rand='.time().'" '.$confirm.'">Вывести дух из минуса'.ShowCost($fixspcost, false, false, false).'</a><br>'; else $sp_fix='';
				if ($fixexp_enable && $val['exp'] < 0) $exp_fix = '<a class="btn btn-mini btn-danger wfull" href="index.php?op=act&n=16&num='.$val['index'].'&rand='.time().'" '.$confirm.'">Вывести опыт из минуса'.ShowCost($fixexpcost, false, false, false).'</a><br>'; else $exp_fix='';
				...
				if ($chname_enable) $ch_n = '<a class="btn btn-mini btn-primary wfull" href="#" onclick="ShowSubActWnd('.$val['index'].', \'Смена создателя вещи\',\'chnam\')">Смена создателя вещи...</a><br>'; else $ch_n = '';
				if ($unbind_enable) $unbind = '<a class="btn btn-mini btn-primary wfull" href="#" onclick="ShowSubActWnd('.$val['index'].', \'Отвязка вещей\',\'prctp\')">Отвязать вещь...</a><br>'; else $unbind = '';
				if ($add_slot_enable) $add_slot = '<a class="btn btn-mini btn-warning wfull" href="#" onclick="ShowSubActWnd('.$val['index'].', \'Добавление ячейки\',\'add_slot\')"><font color="#000000">Добавить ячейку...</font></a><br>'; else $add_slot = '';
				if ($add_distance_enable) $add_distance = '<a class="btn btn-mini btn-warning wfull" href="#" onclick="ShowSubActWnd('.$val['index'].', \'Увеличение дальности\',\'add_distance\')"><font color="#000000">Увеличить дальность...</font></a><br>'; else $add_distance = '';			
				if ($val['buff']) $buff_but = '<a class="btn btn-mini btn-inverse wfull" href="#" onclick="ShowSubActWnd('.$val['index'].', \'Временные бафы\',\'show_buffs\')"><i class="icon icon-color icon-flag"></i> Купить временный баф...</a>'; else $buff_but = '';
				if ($val['skill']) $skill_but = '<a class="btn btn-mini btn-inverse wfull" href="#" onclick="ShowSubActWnd('.$val['index'].', \'Покупка скиллов\',\'show_skills\')"><i class="icon icon-color icon-tag"></i> Купить скилл...</a>'; else $skill_but = '';			
	...
	<span class="well span3 shop-block pers-block">
		...
		<hr>
		<!-- Role info section -->
		Часы ПК: <font color="#ff0000">%s</font><br>
		Убил белых: <font color="#ffff00">%s</font><br>
		Онлайн: <font color="#00ff00">%s</font><br>
		Выполнено квестов: <font color="#00ffff">%s</font><br>
		<hr>		
		<!-- Ban info section -->
		%s
		<!-- Buffs section -->			
...
',
	$val['index'], $class[$val['cls']], ($val['gender'])?'f':'', $val['cls'], htmlspecialchars($val['name']), $val['level'], $faction_txt, $pink, $val['pk_count'], $time, $val['task_count'], $forbid_txt, $active_buffs, $n_pass, $sp_fix, $exp_fix, $lvlupbut, $dungeon_fix, $ch_n, $unbind, $add_slot, $add_distance, $buff_but, $skill_but
);
