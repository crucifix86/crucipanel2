...
function ShowSection($n){
	global $init_itemid, $db, $config_tbl_name;
	...
	while ($row = mysqli_fetch_assoc($res)){
		...
		switch ($row['type']) {
			...
			case 'text':
				$wdh = '';
				if ($n == 5) $wdh = ' style="width:600px; max-height: 300px; overflow: auto"';
			...
			case 'select':
				...
				break;
            case 'password':
                $e = '<input type="password" name="'.$name.'" value="'.$row['value'].'">';
                break;
			default:
...
//clearstatcache();
$ext_needed = array('bcmath', 'curl', 'iconv', 'json', 'mbstring', 'openssl', 'mysqli', 'gd');
foreach ($ext_needed as $i => $val){
    if (!extension_loaded($val)) {
        ...
        echo GetErrorTxt(88,'Установите расширение php'.$prefix.'-'.$val);
    }
}
...
?>
						<ul class="nav nav-tabs" id="myTab">							
							...
							<li><a href="#buffs" onclick="show_save_but(false)">ЛК Бафы</a></li>
							<li><a href="#skills" onclick="show_save_but(false)">Скиллы</a></li>
						...
						<div id="myTabContent" class="tab-content">							
							...
							<div class="tab-pane" id="buffs">
								<?php require_once 'config_buffs.php';?>
							</div>
							<div class="tab-pane" id="skills">
								<?php require_once 'config_skills.php';?>
							</div>
		...
		<div class="row-fluid">
			<div class="box span6">
				...
				<div class="box-content">
					...
					<ul>
						<li>конвертируйте файл <code>iconlist_ivtrm.dds</code><font color="#808080">(из surfaces/iconset)</font> в <code>iconlist_ivtrm.png</code> с помощью утилиты <code>DDSConverter.exe</code></li>
						<li>залейте файлы <code>iconlist_ivtrm.png</code> и <code>iconlist_ivtrm.txt</code> в папку <code><?=$path?>/</code> клиент части</li>
					</ul>					
					<span class="label label-important">Обратите внимание!</span>
					<ul align="left" type="disc">
						<li>Если на хостинге стоит ограничение по объему памяти, воспользуйтесь <a href="http://cp.alexdnepro.net/icon_maker.php" target="_blank">внешним генератором</a></li>
...
сгенерированный SQL файл импортируйте в базу данных клиент части</li>
			...
			<div class="box span6">
				<div class="box-header well" data-original-title>
					<h2><i class="icon icon-color icon-refresh"></i> Обновление списка иконок для скиллов</h2>
				</div>
				<div class="box-content">
					<p>Подготовка файлов:</p>
					<ul>
						<li>конвертируйте файл <code>iconlist_skill.dds</code><font color="#808080">(из surfaces/iconset)</font> в <code>iconlist_skill.png</code> с помощью утилиты <code>DDSConverter.exe</code></li>						
						<li>залейте файлы <code>iconlist_skill.png</code> и <code>iconlist_skill.txt</code> в папку <code><?=$path?>/</code> клиент части</li>
					</ul>
					<span class="label label-important">Обратите внимание!</span>
					<ul align="left" type="disc">
						<li>Если на хостинге стоит ограничение по объему памяти, воспользуйтесь <a href="http://cp.alexdnepro.net/icon_maker.php" target="_blank">внешним генератором</a></li>
...
сгенерированный SQL файл импортируйте в базу данных клиент части</li>
					...
					<center>
					<a href="index.php?cron_act=make_skill_icons&cron_passw=<?=$cron_act_passw?>" class="btn btn-primary" target="_blank">Обновить иконки скиллов</a></center>
				</div>
			</div>					
		</div>		
		<div class="row-fluid">
