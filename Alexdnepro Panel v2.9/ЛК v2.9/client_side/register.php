...
if (isset($_GET['resendmail']) && isset($_SESSION['login']) && ($_SESSION['login'] != '') && !preg_match($login_filter, $_SESSION['login']) && (strlen($_SESSION['login']) >= $login_min_len && strlen($_SESSION['login']) <= $login_max_len ) && $email_confirm) {
	...
	if ($a['errorcode'] != 0) {
		...
		$text.=sprintf('<p>Чтобы активировать аккаунт, перейдите по следующей ссылке: <a href="%s">%s</a></p>', $confirm_link, $confirm_link);
		$text.='<br><p><i>Отвечать на это письмо не нужно, оно было сгенерировано автоматически, а робот не любит читать письма ;).</i></p></body>';	
...
if ($goreg==1 && $register_active) {
	//Проверки
	if(!isset($_SESSION['captcha_keystring']) || $_SESSION['captcha_keystring'] != $postkapcha){
		$error=1;
	...
	if ($error==0){
		...
		if (substr($result, 0, 2)=='ok') {
			...
			$text = '<body><div id="text" align="left">
		        <h3>Данные, указанные при регистрации аккаунта:</h3>
		        <table width="100%" cellpadding="2" cellspacing="0" border="0" align="center">';
			$text.=getdata('Логин','<font color="#0000ff">'.$postlogin.'</font>');
			$text.=getdata('Пароль',$postpass);
			$text.=getdata('Вопрос',$postquestion);
			$text.=getdata('Ответ',$postanswer);
			$text.=getdata('Имя',$postname);
			$text.='</table></div><br>';
			if ($email_confirm) {
				$confirm_link = GetLink().'register.php?activate='.mycrypt(encode(sprintf('%d|%s|%d', $servid, $postlogin, $realid)));
				$text.=sprintf('<p>Чтобы активировать аккаунт, перейдите по следующей ссылке: <a href="%s">%s</a></p></br>', $confirm_link, $confirm_link);
			}
			$text.='
			<p>Приглашайте друзей регистрироваться на проекте по реферальной ссылке и получайте бонусы!</p>
			<p>Ваша реферальная ссылка: <a href="'.$reflink.'" target="_blank">'.$reflink.'</a></p><br>
...
<div class="row-fluid">
	<div class="well box">
		...
		<div class="box-content">
			<?php			
			if ($register_active) {
			if (($goreg==1)&&($error==0)) echo '<div class="alert alert-success">Поздравляем, аккаунт <b><font color=#0000ff>'.$lg.'</font></b> успешно зарегистрирован!<br> Данные от аккаунта отправлены Вам на почту.'.$act_txt.'</div>'; else
			...
			if (($goreg==3)&&($error==0)) echo '<div class="alert alert-success">Письмо с ссылкой активации аккаунта <b><font color=#0000ff>'.$lg.'</font></b> успешно отправлено.</div>'; else
			...
			<form class="form-horizontal" id="register" name="reg" method="POST">
			  ...
			  <fieldset>			    
			    ...
			    <div class="control-group">
			      ...
			      <div class="controls">
			        <input type="text" value="" name="kapcha" id="kapcha" class="inputc" maxlength="4"> <img src="captcha/?<?php echo session_name()?>=<?php echo session_id()?>&tt=<?=doubleval(microtime())?>" id="image" onClick="iload()" border="0">
			        <p class="help-block">Введите код, который Вы видите на картинке</p>
			      </div>
			    </div>
